 ...
 */
class TwitterBootstrapPaginatorHelper extends AppHelper {
 ...
 */
	public function prev($title = '<< Previous', $options = array(), $disabledTitle = null, $disabledOptions = array()) {
		if($options['url']['page'] > 1){
			$options['url']['page'] = intval($options['url']['page']) - 1;
		}else{
			$options['url']['page'] = null;
		}
		$defaults = array(
			'rel' => 'prev'
		);
		$options = array_merge($defaults, (array)$options);
		$disabledOptionsDefaults = array('tag'=>'li');
		$disabledOptions = array_merge($disabledOptionsDefaults, (array)$disabledOptions);
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					...
					$out .= $this->Html->tag($tag, $this->link($i,'#'), array('class' => $currentClass));
				} else {
